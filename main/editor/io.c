...
uint8_t read_byte(bun_file_s* pfile, uint8_t* result){
    return WIN_CHAR_LEN == file_read(pfile, result, sizeof(uint8_t), WIN_CHAR_LEN);
}
...
uint8_t read_word(bun_file_s* pfile, uint16_t* result){
    ...
    numRead = file_read(pfile, buf, sizeof(uint8_t), WIN_WORD_LEN);
...
uint8_t read_dword(bun_file_s* pfile, uint32_t* result){
    ...
    numRead = file_read(pfile, buf, sizeof(uint8_t), WIN_DWORD_LEN);
